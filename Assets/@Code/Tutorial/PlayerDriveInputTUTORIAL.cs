...
public class PlayerDriveInputTUTORIAL : MonoBehaviour {
    public static PlayerDriveInputTUTORIAL current;
    ...
    public bool isPickups = true; //in-game
    ...
    [SerializeField] private PlayerInteractionTUTORIAL pi;
    ...
    private void Start() {
        pi = GetComponent<PlayerInteractionTUTORIAL>();
    ...
    public void SetIsDriving(bool newIsDriving, CarController newCarCon, Transform seat) {
        ...
        carCon = newCarCon;
        if(JeepneyPanel.current) JeepneyPanel.current.SetCarcon(carCon);
        if(carCon) {
            carCon.moveInput = 0;
            carCon.steerInput = 0; 
        }
