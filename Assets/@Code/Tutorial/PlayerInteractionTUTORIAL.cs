...
public class PlayerInteractionTUTORIAL : MonoBehaviour {
    ...
    public bool hudToggleDown;
    public bool lMouseHold;
    public bool rMouseHold;
    ...
    public void GrabAllFromStorage(StorageHandler storage) {
        for(int i = 0; i < maxItemsOnHand; i++) {
            ...
            if(rightHand.childCount == maxItemsOnHand) {
                NotificationManager.current.NewNotif("HANDS FULL", "You cannot hold any more items!");
                break;
            }
    ...
    private void OnTriggerEnter(Collider other) {
        ...
        if(!playerDestUI.isIn && !inDrop && go.layer == layerDrop) { // && areaUI.text != other.name
            ...
            if(go.GetComponent<DropSpot>().isIllegal) {
                ...
                Tutorial.current.InIllegalArea();
            } else {
